...
void main() {
  ...
  group('Disciplinas Routes - GET', () {
    ...
    test(
      'GET /disciplinas/professor/:id - deve listar disciplinas do professor',
      () async {
    ...
    test(
      'GET /disciplinas/professor/:id - deve retornar lista vazia para professor sem disciplinas',
      () async {
  ...
  group('Disciplinas Routes - POST', () {
    test(
      'POST /disciplinas - deve criar nova disciplina com dados completos',
      () async {
        final response = await http.post(
          Uri.parse('$baseUrl/disciplinas'),
          headers: {'Content-Type': 'application/json'},
    ...
    test(
      'POST /disciplinas - deve criar disciplina sem descrição (opcional)',
      () async {
        final response = await http.post(
          Uri.parse('$baseUrl/disciplinas'),
          headers: {'Content-Type': 'application/json'},
    ...
    test('POST /disciplinas - deve rejeitar disciplina sem nome', () async {
      final response = await http.post(
        Uri.parse('$baseUrl/disciplinas'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'cor': '#FF0000', 'professor_id': professorId}),
      );
    ...
    test(
      'POST /disciplinas - deve rejeitar disciplina sem professor_id',
      () async {
        final response = await http.post(
          Uri.parse('$baseUrl/disciplinas'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'nome': 'Teste', 'cor': '#FF0000'}),
        );
    ...
    test(
      'POST /disciplinas - deve rejeitar professor_id inexistente',
      () async {
        final response = await http.post(
          Uri.parse('$baseUrl/disciplinas'),
          headers: {'Content-Type': 'application/json'},
    ...
    test(
      'POST /disciplinas - deve aceitar cores em diferentes formatos',
      () async {
  ...
  group('Disciplinas Routes - PUT', () {
    ...
    test('PUT /disciplinas/:id - deve atualizar apenas nome', () async {
      final response = await http.put(
        Uri.parse('$baseUrl/disciplinas/$disciplinaId'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'nome': 'Apenas Nome Novo'}),
      );
    ...
    test(
      'PUT /disciplinas/:id - deve retornar 404 para ID inexistente',
      () async {
        final response = await http.put(
          Uri.parse('$baseUrl/disciplinas/99999'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'nome': 'Teste'}),
        );
  ...
  group('Disciplinas Routes - DELETE', () {
    ...
    test(
      'DELETE /disciplinas/:id - deve retornar 404 para ID inexistente',
      () async {
        final response = await http.delete(
          Uri.parse('$baseUrl/disciplinas/99999'),
        );
    ...
    test('DELETE /disciplinas/:id - deve rejeitar ID inválido', () async {
      final response = await http.delete(Uri.parse('$baseUrl/disciplinas/abc'));

      expect(response.statusCode, equals(400));
    });

    test(
      'DELETE /disciplinas/:id - deve deletar em cascata (atividades e notas)',
      () async {
        // Criar disciplina
